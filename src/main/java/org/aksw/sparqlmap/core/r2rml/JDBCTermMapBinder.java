...
import static org.aksw.sparqlmap.core.r2rml.R2RMLHelper.*;
...
public class JDBCTermMapBinder {
  ...
  public JDBCTermMap bind(TermMap termMap, FromItem fromItem) {
    ...
    // set the compatibility checker
    jTermMap.setCompChecker(new SimpleCompatibilityChecker(jTermMap, dbaccess, dth));
  ...
  private List<Expression> templateToResourceExpression(List<TermMapTemplateStringColumn> list, FromItem fi, DataTypeHelper dth) {

    List<Expression> newExprs = Lists.newArrayList();
    
    // now create a big concat statement.
    for (TermMapTemplateStringColumn ttsc: list) {
