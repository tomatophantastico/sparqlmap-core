...
public class Automapper {
  ...
  public Model getMydbData(String catalog, String schema) throws SQLException {
    ...
    try {
      ...
      ResultSet tables = md.getTables(catalog, schema, "%", TABLE_TYPES);

      //catalogs.beforeFirst();
      while (tables.next()) {
        String tableName = tables.getString(3); // "TABLE_CATALOG"
        mapTable(tableName);
        // PK fields
        ResultSet primaryKeyRecordSet = md.getPrimaryKeys(null, null, tableName);

        // Map PKs
        mapPrimaryKey(primaryKeyRecordSet, catalog, schema, tableName);
  ...
  private String urlEncode(String tableName) throws UnsupportedEncodingException {
    return UrlEscapers.urlPathSegmentEscaper().escape(tableName);
  }
   ...
   * @throws SQLException
   */
  int getPrimaryKeyCount(String catalog, String schema, String tableName) throws SQLException {
    java.sql.ResultSet primaryKeyRecordSet = null;
    int rowcount = 0;
    primaryKeyRecordSet = md.getPrimaryKeys(catalog, schema, tableName);
   ...
   * @throws UnsupportedEncodingException
   */
  void mapPrimaryKey(java.sql.ResultSet primaryKeyRecordSet, String catalog, String schema, String tableName) throws UnsupportedEncodingException,
    SQLException {
    // Count PKs
    int primaryKeysCount = getPrimaryKeyCount(catalog, schema, tableName);
  ...
  void mapNoPrimaryKey(String tableName) throws SQLException, UnsupportedEncodingException {
    ...
    List<String> colsToAdd = Lists.newArrayList(); 

    while (r.next()) {
      String columnName = r.getString(4);
      colsToAdd.add(tableNameEncoded + "={\"" + columnName + "\"}");
    }
    compositePrimaryKeyString = Joiner.on(compPkSep).join(colsToAdd);
